...
\definecolor{darkgreen}{rgb}{0,0.8,0} % The smaller the number the darker it is
...
We will implement toy VMC and DMC programs in Python to calculate the $^1S$ ground and $^3S$ first excited state
of Helium (or more generally any 2-electron atom/ion).
VMC will give a variational upper bound for both states, whereas DMC will give the exact energies (aside
from statistical error, which can be made arbitrarily small by running longer).
...
and Laplacians numerically and prints out the difference between the numerical evaluation and the analytic.
...
Hint: Look at the last term of $\EL$.
}
...
Fill out the top line of Table~\ref{H-}.
}
...
{\color{blue}
\textbf{Q4. Try better wavefunction for H$^-$:}\\ 
...
of being accepted.  This is a problem even in VMC, but can becomes a more serious problem in DMC
...
Use the wavefunction in Eqs.~\ref{Slater_Jastrow}, with
